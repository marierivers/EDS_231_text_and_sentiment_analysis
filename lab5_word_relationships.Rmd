...
```{r pdf_import}
#setwd('data/EJ/')
#data_path <- here("data", "EJ") 
#files <- list.files(data_path, pattern = "pdf$")
...
```{r co-occur_plots}
word_pairs %>%
  filter(n >= 100) %>% # n was originally 70
...
```{r}
# xxx...not values in the idf and tf_idf columns???
...
```{r}
# one word tokens
# keyness is a quanteda thing
keyness <- textstat_keyness(dfm, target = 1) # target = 1 refers to the first document (ie the 2015 report)
textplot_keyness(keyness)
# accounting for baseline frequency, "climate" occurred more than expected based on baseline frequency
# words with negative chi squared occur less frequently in 2015 than expected based on all other years
# reference is all the reports except 2015
```
```{r}
keyness <- textstat_keyness(dfm, target = 2) # target = 2 refers to the second document (ie the 2016 report)
textplot_keyness(keyness)
```
...
```{r}
# bigrams
keyness <- textstat_keyness(dfm2, target = 1) # target = 1 refers to the first document (ie the 2015 report)
textplot_keyness(keyness)
```
